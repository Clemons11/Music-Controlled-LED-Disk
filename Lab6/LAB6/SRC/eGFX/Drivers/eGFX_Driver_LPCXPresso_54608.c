...
void eGFX_InitDriver()
{
  ...
  CLOCK_AttachClk(kMCLK_to_LCD_CLK);
	...
	eGFX_ImagePlaneInit(&eGFX_BackBuffer,(uint8_t *)SDRAM->eGFX_FrameBuffer2,eGFX_PHYSICAL_SCREEN_SIZE_X, eGFX_PHYSICAL_SCREEN_SIZE_Y,eGFX_IMAGE_PLANE_16BPP_565);
...
void eGFX_Dump(eGFX_ImagePlane *Image)
{
 //	eGFX_WaitForVSync();
 	...
 	if(Image->Data == &SDRAM->eGFX_FrameBuffer2[0])
		{
			Image->Data  = &SDRAM->eGFX_FrameBuffer1[0];
			LCD->UPBASE  = (uint32_t)(&SDRAM->eGFX_FrameBuffer2[0]);
		}
		else
		{
			Image->Data  = &SDRAM->eGFX_FrameBuffer2[0];
			LCD->UPBASE = (uint32_t)(&SDRAM->eGFX_FrameBuffer1[0]);
		}
